...
Statement desugar((Statement)`test <Expression x> should be <Expression y>;`) {
  	return (Statement)`(function(actual, expected) {if(actual != expected) console.log("Test failed; expected: " + expected + "; got: " + actual);})(<Expression x>, <Expression y>);`;
  }
...
Expression desugar((Expression)`<Id param> =\> <Expression body>`) {
	return (Expression)`(function (_this) { 
						...
						'		return replaceThis(<Expression body>); 
						'	}; 
						'})(this)`;
}
